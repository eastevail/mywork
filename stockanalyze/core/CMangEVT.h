...
public:
    ...
    CRtpQueue* GetInstanceRtpQue(char *sn);//get a rtp queue according to sn
    int DelRtpQue(char *sn);//delete a rtp queue according to sn
...
//    CEventBase* GetIpcAccordSN(char* sn);//
    ...
    map<string,CRtpQueue*> m_ManageRtpQueue;
