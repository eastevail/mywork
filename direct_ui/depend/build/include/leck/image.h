...
typedef void (*AfterDrawImage) (void *ctx);
...
DFBResult lite_set_image_draw_after(LiteImage *image, AfterDrawImage callback, void *ctx);

DFBResult lite_get_image_size(LiteImage *image,int* width ,int* height);


DFBResult lite_destroy_image(LiteImage *image);
