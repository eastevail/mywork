...
typedef DFBResult (*LiteLoopCheckfunFunc) (void *data);
...
LiteLoopCheckfunFunc check_trigger_updatefunCb ; /*added for topbang check trigger in windows loop */
 void*    			check_trigger_updateData ;
...
typedef enum {
     TimeOut_OneTime        = 0,             /**< timer type, only one-shot */
     TimeOut_EveryTime      = 1,              /**< timer type trigger every period unless delete it */
     TimeOut_EveryTime_Priority = 2
}TimeOut_Type_E;
...
typedef struct _LiteWindowTimeout {
  ...
  int period;                                                   /**< period time that triggers this timeout */
  TimeOut_Type_E type;                                          /**< timer type */
 ...
 * @return DFBResult            Returns DFB_OK if successful.
 */
DFBResult lite_enqueue_window_timeout(int timeout, TimeOut_Type_E type, LiteTimeoutFunc callback,
