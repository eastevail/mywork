/*
 * avDataPro.cpp
 *
 *  Created on: 2016年8月6日
 *      Author: lixiang 21588
 */

#include "avDataPro.h"

avDataPro::avDataPro()
{
	// TODO Auto-generated constructor stub

}

avDataPro::~avDataPro()
{
	// TODO Auto-generated destructor stub
}

int avDataPro::getOneVideoFrameFromDri(int rot_num,uint8_t** start, uint32_t *len)
{
	return 0;
}
int avDataPro::getOneAudioFrameFromDri(uint8_t* start, uint32_t *len)
{
	return 0;
}
int avDataPro::displayOneVideoFrame(uint8_t* start, uint32_t len)
{
	return 0;
}
int avDataPro::displayOneAudioFrame(uint8_t* start, uint32_t len)
{
	return 0;
}
int avDataPro::setDisplayZoom(int level)
{
	return 0;
}
int avDataPro::enableVideoForPhoto()
{
	return 0;
}
int avDataPro::disableVideoForPhoto()
{
	return 0;
}
int avDataPro::enableVideoForRec()
{
	return 0;
}
int avDataPro::disableVideoForRec()
{
	return 0;
}
int avDataPro::enableVideoForCodec()
{
	return 0;
}
int avDataPro::disableVideoForCodec()
{
	return 0;
}











