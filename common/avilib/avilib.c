...
void AVI_set_audio(avi_t *AVI, int channels, long rate, int bits, int format, char *compressor)
{
   /* may only be called if file is open for writing */
   ...
   AVI->a_fmt   = format;
    ...
    memcpy(AVI->a_compressor,compressor,4);
    AVI->a_compressor[4] = 0;
}
...
#define AVIF_COPYRIGHTED        0x00020000
   ...
   if (AVI->a_chans && AVI->audio_bytes)
   {
	   ...
	   OUT4CC ("auds");
	   //OUT4CC ("\0\0\0\0");
	   OUT4CC (AVI->a_compressor);    /* Handler */
	   OUTLONG(0);             /* Flags */
	   OUTLONG(0);             /* Reserved, MS says: wPriority, wLanguage */
	   OUTLONG(0);             /* InitialFrames */
...
avi_t *AVI_open_input_file(const char *filename, int getIndex)
{
   ...
   for(i=0;i<hdrl_len;)
   {
      ...
      if(strncasecmp(hdrl_data+i,"strh",4)==0)
      {
         ...
         {
            memcpy(AVI->a_compressor,hdrl_data+i+4,4);
            AVI->a_compressor[4] = 0;         
